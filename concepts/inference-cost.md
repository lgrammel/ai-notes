...
[Semantic caching](./semantic-caching.md) goes further by skipping inference entirely for queries that are semantically similar to previously cached responses, eliminating token costs altogether for cache hits. [Speculative decoding](./speculative-decoding.md) does not reduce token cost directly but improves throughput per GPU, which lowers the serving cost per token for [inference providers](./inference-provider.md) and can translate into lower pricing or higher capacity on the same hardware.
