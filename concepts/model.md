...
Large language models are typically neural networks trained to predict the next token using large text and multimodal datasets; after [training](./training.md), they are run during [inference](./inference.md) and are commonly used in chatbots and [agents](./agent.md). In this repository, models are assumed to follow the [transformer architecture](./transformer-architecture.md).
