...
Prompt caching is a provider-side optimization that reuses computation from shared prompt prefixes, reducing [latency](./latency.md) and [inference cost](./inference-cost.md) for subsequent requests that start with the same [token](./token.md) sequence. Unlike [semantic caching](./semantic-caching.md), which returns stored responses for semantically similar queries without invoking the model, prompt caching still runs [inference](./inference.md) but skips redundant processing of the shared prefix.

## Details

During [inference](./inference.md), the model first processes all input tokens (the prefill phase), building up internal key-value states (KV-cache) for each layer. Prompt caching stores this KV-cache keyed by the exact token prefix. When a later request starts with the same prefix, the [inference provider](./inference-provider.md) loads the cached state instead of recomputing it, producing faster time to first token and lower cost. Cache hits typically require an exact token-level prefix match - even small changes early in the [prompt](./prompt.md) invalidate the cache - though some providers apply more flexible matching strategies.
