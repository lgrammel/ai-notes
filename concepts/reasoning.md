...
Reasoning tokens add to [inference cost](./inference-cost.md) and [latency](./latency.md), particularly for models that produce extended reasoning traces before the final answer. In [agent](./agent.md) systems, reasoning is the capability exercised at each step of the [agent](./agent.md) loop - evaluating the current [context](./context.md), selecting a [tool](./tools.md) call, and determining when a final response is ready. [Planning](./planning.md) is an application of reasoning focused specifically on structuring multi-step execution: decomposing a task into steps and determining their order. Because reasoning governs an agent's [planning](./planning.md) and action selection, it is a target for [goal manipulation](../threats/goal-manipulation.md) attacks that attempt to redirect the agent's objectives.
