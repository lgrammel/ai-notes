...
Claude Code is a [coding agent](../concepts/coding-agent.md) operating as a [local coding agent](../concepts/local-coding-agent.md) in the developer's terminal, combining a [multi-turn](../concepts/multi-turn-conversation.md) [agent](../concepts/agent.md) loop with [filesystem](../concepts/filesystem-agent.md) and [shell](../concepts/shell-tool.md) access, [subagent](../concepts/subagent.md) delegation, [human-in-the-loop](../concepts/human-in-the-loop.md) approval, [reasoning](../concepts/reasoning.md) traces, and [skills](../concepts/skill.md) loaded from the workspace.

## Details

Unlike IDE-embedded [copilot interfaces](../concepts/copilot-interface.md), Claude Code runs as a standalone CLI process in the developer's terminal. This makes it environment-agnostic - it works with any editor, build system, or workflow that operates on the filesystem and shell. The same CLI interface also enables a headless mode for non-interactive automation (CI/CD pipelines, scripted workflows), where [human-in-the-loop](../concepts/human-in-the-loop.md) approval is absent and the agent runs fully autonomously.
...
- [Skills](../concepts/skill.md) (instruction files loaded from workspace, e.g. `CLAUDE.md`)
...
Claude Code runs unsandboxed in the developer's local environment with the developer's full permissions. There is no [sandbox](../concepts/sandbox.md) boundary - the agent can access anything the developer can, including credentials, SSH keys, and other projects on the filesystem. [Human-in-the-loop](../concepts/human-in-the-loop.md) approval via [tool execution approval](../concepts/tool-execution-approval.md) is the primary safety mechanism rather than technical containment. The developer configures which tool categories require approval, with read-only operations (file reads, searches) typically auto-approved and write operations (file edits, shell commands) requiring confirmation.

The filesystem is both workspace and instruction source: [skill](../concepts/skill.md) files (e.g. `CLAUDE.md`) are discovered and loaded into the agent's [context](../concepts/context.md) by the [agent runtime](../concepts/agent-runtime.md). A compromised repository can alter the agent's behavioral instructions before the developer interacts with it, through [context poisoning](../threats/context-poisoning.md) via crafted skill files.

The CLI-based interface means the developer reviews proposed actions in a terminal rather than a rich diff view, which can affect the fidelity of human review for complex multi-file changes. In headless mode (CI/CD, scripted automation), the human review boundary is absent entirely - the agent runs autonomously with whatever permissions the pipeline grants, making every other defense layer (input validation, permission scoping, output verification) load-bearing.
...
- **Human-in-the-loop + high action volume**: Extended coding sessions produce many file edits and shell commands. The propose-review-execute loop is the primary defense, but its effectiveness degrades as the developer's review attention diminishes over a long session.
- **Headless mode + no sandbox**: In CI/CD or scripted automation, the agent operates without human oversight and with pipeline-level permissions. Any successful [prompt injection](../threats/prompt-injection.md) or [context poisoning](../threats/context-poisoning.md) leads directly to autonomous execution of attacker-controlled actions without a human review step.
...
- [System prompt extraction](../threats/system-prompt-extraction.md) - tricking the model into revealing its system instructions or loaded skill file contents through conversation or tool interactions
...
- [Context poisoning](../threats/context-poisoning.md) - manipulated workspace files or skill files (e.g. `CLAUDE.md`) that alter agent behavior when loaded into context, enabling an attacker to control the agent through a compromised repository
