...
- If every agent session produces a demonstration artifact, reviewers may stop watching them carefully - the same [approval fatigue](../threats/approval-fatigue-exploitation.md) dynamic that affects code review, shifted to a medium that may be worse for sustained attention. A polished demo with screenshots and passing commands looks more convincing than a code diff, which means a fatigued reviewer is more likely to approve without scrutiny - the artifact's persuasiveness works against the skepticism it is supposed to enable.
...
- The approach is biased toward user-facing features where command execution and screenshots produce meaningful evidence. Backend services, data pipelines, and ML models are not naturally demo-able through this medium - their correctness depends on properties (throughput, data integrity, model accuracy) that do not reduce to visual or textual command output, limiting the technique's applicability across software categories.
- Demonstration artifacts verify a point-in-time snapshot of behavior. As the software evolves, previously generated artifacts become misleading records that imply tested behavior still holds when it may not, and there is no built-in mechanism to invalidate or regenerate stale artifacts when the underlying code changes.
