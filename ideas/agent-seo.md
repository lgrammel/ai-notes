...
Awareness can be gained organically (usage data enters [training](../concepts/training.md) sets), through content optimized for agent consumption at [inference](../concepts/inference.md) time (loaded into [context](../concepts/context.md)), or through commercial engagement with [model developers](../concepts/model-developer.md) to influence training directly. [Model familiarity bias](../concepts/model-familiarity-bias.md) drives the underlying dynamic: agents systematically favor well-represented offerings, so in crowded domains a useful but unknown offering can lose to a mediocre but familiar one.
