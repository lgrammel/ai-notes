...
Warning signs include team members hesitating to make changes for fear of unintended consequences, growing reliance on tribal knowledge held by one or two people, and a sense that the system is becoming a black box. Common mitigations center on deliberate slowdowns: at least one human fully understanding each AI-generated change before it ships, documenting intent (why, not just what), and rebuilding shared understanding through code reviews, retrospectives, and practices like pair programming, refactoring, and [TDD](../ideas/tdd-as-agent-constraint.md). [Supervisory engineering](./supervisory-engineering.md) formalizes the skill set needed to maintain understanding at agent-driven velocity - decomposing problems, calibrating trust in output, and preserving architectural coherence. Structural mitigations complement these: strong type systems and formal verification reduce the surface area of unknown unknowns by catching classes of errors mechanically, providing guardrails that hold even when human understanding erodes.
...
- In a [dark software factory](../ideas/dark-software-factory.md) model where humans never touch the code and quality is ensured through mechanical enforcement and agent-driven validation, cognitive debt as traditionally defined becomes irrelevant - the shared theory shifts from understanding code to understanding specifications and the harness infrastructure.
...
**High.** The core observation - that velocity without understanding is unsustainable - is well-supported by decades of software engineering experience, and AI agents visibly amplify the dynamic through higher code throughput, approval fatigue, and ambiguous authorship. The counterarguments identify measurement difficulty (cognitive debt is hard to quantify until it becomes paralyzing), but this is a limitation of the metric, not of the concept's validity - the same is true of technical debt, which remains a useful and widely applied framework despite similar measurement challenges.
