...
- When code no human has read breaks in production, diagnosing and fixing it is extremely difficult. Delegating debugging to agents creates a trust recursion - the same verification gap that required the dark factory model now applies to incident response, and each layer of agent-driven diagnosis inherits the reliability limits of the layer it is trying to fix.
- The approach assumes that specifications are less ambiguous than code, but natural language specifications are notoriously ambiguous. Bugs may shift from implementation errors to specification misinterpretations, trading one class of defect for another without reducing the total defect rate - and specification bugs are harder to catch because they look like correct behavior relative to the spec.
